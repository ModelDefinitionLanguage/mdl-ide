...
//        performConvert(source, targetDir);
   ...
   def void performConvert(IFile src, IPath dest) {
...
//       val Resource resource = resourceSet.getResource(URI.createURI("file:///" + src.rawLocationURI), true);
       val Resource resource = resourceSet.getResource(URI.createURI(src.rawLocationURI.toString), true);
...
//	   baseName = baseName.replace(".mdl", ".xml");
       for (mog : mcl.getMogObjects){
           ...
           printOutputFile(newDest, converted.toString());
...
//           val is = new ByteArrayInputStream(converted.toString.getBytes(StandardCharsets.UTF_8))
//           dest.setContents(is, true, false, null)
       }
   }
   
   def void printOutputFile(IPath outputDirectory, String text) {
       val outputFile = outputDirectory.toFile;
