...
class Mdl2PharmML{
	...
	protected def print_mdef_CovariateModel(String symbId)'''
		<Covariate symbId="«symbId»">
			<Continuous>
				<Transformation>
					<math:Equation>
						«mathPrinter.print_ct_SymbolRef(symbId)»
					</math:Equation>
				</Transformation>
			</Continuous>	
		</Covariate>
		...
		«IF s.randomList != null»
			...
			«IF level.length > 0»
				<ct:VariabilityReference>
					«mathPrinter.print_ct_SymbolRef(level)»
				</ct:VariabilityReference>
			«ENDIF»
		«ENDIF»
	'''
	...
	def getGetRandomVariableByVariability(SymbolDeclaration v) {
		...
		var type = mathPrinter.getAttribute(v.expression.list.arguments, AttributeValidator::attr_re_type.name);
			...
			if (o.modelObject != null){
				for (b: o.modelObject.blocks){
					if (b.randomVariableDefinitionBlock != null){
						for (s: b.randomVariableDefinitionBlock.variables){
							if (s.randomList != null){
								var variance = mathPrinter.getAttribute(s.randomList.arguments, attrName);	
		...
		if (parameters != null){
			...
			for (symbol: parameters.symbols){
				if (symbol.symbolName != null){
					rowNames = rowNames + mathPrinter.print_ct_SymbolRef(objName, symbol.symbolName.name);
				}
			}	
			val matrixType = mathPrinter.getAttribute(arguments, AttributeValidator::attr_re_type.name);
	...
	def print_mdef_Same(String objName, SameBlock same){
		if (same.parameters != null){
			...
			for (symbol: same.parameters.symbolNames){
				if (symbol != null){
					rowNames = rowNames + mathPrinter.print_ct_SymbolRef(objName, symbol.name);
				}
			}	
			val sameName = mathPrinter.getAttribute(same.arguments, AttributeValidator::attr_name.name);
						...
						if (o.parameterObject != null){
							for (b: o.parameterObject.blocks){
								if (b.variabilityBlock != null){
									for (c: b.variabilityBlock.statements){
										if (c.diagBlock != null){
											val name = mathPrinter.getAttribute(c.diagBlock.arguments, AttributeValidator::attr_name.name);
											if (name.equals(sameName)) {
												parameters = c.diagBlock.parameters;
												matrixType = mathPrinter.getAttribute(c.diagBlock.arguments, AttributeValidator::attr_re_type.name);
											}
										}
										if (c.matrixBlock != null){
											val name = mathPrinter.getAttribute(c.matrixBlock.arguments, AttributeValidator::attr_name.name);
											if (name.equals(sameName)) {
												parameters = c.matrixBlock.parameters;
												matrixType = mathPrinter.getAttribute(c.matrixBlock.arguments, AttributeValidator::attr_re_type.name);
											}
										}
									}
								}
